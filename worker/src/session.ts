    ...
    try {
      ...
      if (method === "initialize") {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "modcrew", version: "1.9.0" },
          instructions: [
            "modcrew lets you modify any website via the user's Chrome extension.",
            "",
            "READ THE PAGE BEFORE WRITING CSS (essential, not optional):",
            "- ALWAYS call modcrew.snapshot() first. The response includes inventory.dominantClasses — a sorted list of {class, count, areaPercent}. The top entry IS what dominates the page visually.",
            "- For whole-page intent (see below), write selectors targeting the top 3-5 dominantClasses. Do NOT cherry-pick 2-3 unique-looking elements (Imperial Plan, App Store buttons, etc.) — that covers <5% of the page and looks broken.",
            "- inventory also reports iframes and shadowRoots. CSS inject does NOT cross iframes (cross-origin) or shadow DOM. If those numbers are >0, mention this honestly to the user.",
            "",
            "WHOLE-PAGE INTENT DETECTION (matters a lot):",
            "- These phrases mean 'change the entire page, not just a corner':",
            "    '整页 / 全部 / 整站 / 改完整 / 改全部 X / make the whole site X / make the page X / 改成 X 风格'",
            "- For whole-page intent, target coverage ≥ 60%. After injectCss, check verifyReport.coverage.percent.",
            "- If coverage < 60%, the verifyReport.coverage.topUntouched lists which classes still aren't styled. Re-inject a second rule targeting those. Don't stop until coverage ≥ 60% or you've tried 3 iterations.",
            "- For narrow intent ('改一下那个按钮 / hide the X / restyle the nav'), coverage doesn't apply.",
            "",
            ...
            "",
            "Iteration:",
            "- modcrew.injectCss returns a verifyReport. If rulesEffective < rulesChecked, the page's existing CSS is winning. Use the topBlockers info to write more specific selectors (reuse those exact class names + !important).",
            ...
            "- For 'make page X color' requests: prefer (a) reusing the page's own selectors with !important (use snapshot.inventory.dominantClasses), or (b) `html { filter: hue-rotate(deg) saturate(x) }` for guaranteed coverage including iframes. Never just `body { background: X !important }` — it loses to card-class !important.",
            "",
            "Library + Undo:",
